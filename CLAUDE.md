...
~~27.~~ ✓ Multi-inference backend — `OaiProvider` in `agent` crate covers Ollama / vllm / OpenRouter (OAI-compatible REST); `AnthropicProvider` stays default; selected via `AGENTD_BACKEND` env var; `AGENTD_OAI_BASE_URL` (default `http://localhost:11434/v1`); `AGENTD_MODEL` override; `/api/models` GET proxies live model list (Ollama tags, etc.); topbar backend badge (green for local); model selector fetches `/api/models` dynamically
