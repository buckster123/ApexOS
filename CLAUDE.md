...
~~38d.~~ ✓ Desktop face widget — `deploy/apex-face.py` `render_face()` ported 1:1 to an HTML5 canvas (`faceInit`/`faceRender` in `desktop-app.js`, native 240×240 logical space scaled to canvas); 7 states (idle/thinking/speaking/alert/listening/sleeping/happy), ~8fps tick; state derived client-side from the bus via a single `window._faceOnEvent` hook in `app.js handleEvent` (wake→listening, agent_text→speaking, tool/approval/council→thinking, turn_complete→idle→sleeping@45s, error/IAQ>150→alert, evolution_applied→happy); `😊 Face` in start menu; gives APEX expressions on hosts without the physical GC9A01A; UI-only, no backend change
