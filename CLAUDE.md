...
~~17.~~ ✓ Agent bodies Phase 6d — `SensorReading` enum + `Event::SensorReading` in core; `/sensor-bridge` WS endpoint; `apex-sensor-bridge` HTTP-polls SensorHead (BME688 BSEC2 + MLX90640 thermal); `AirQuality` + `ThermalFrame` variants; `sensorhead-dashboard.service`; IAQ > 150 alert routing; smoke: IAQ/T/RH/P + thermal frame flowing live in agentd every 30s
